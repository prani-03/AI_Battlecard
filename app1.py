...
# Create a marquee for the logos using HTML
marquee_html = """
...
# Create logo images with captions
logo_images = "".join(
    f'<div style="display: inline-block; text-align: center; margin-right: 20px;">'
    f'<img src="{competitor["logo"]}" width="150" style="vertical-align: middle;" onerror="this.style.display=\'none\'"/>'
    f'<div>{competitor["name"]}</div>'
    f'</div>'
    for competitor in competitor_logos
)

# Display the marquee with logos
st.markdown(marquee_html.format(logo_images), unsafe_allow_html=True)
